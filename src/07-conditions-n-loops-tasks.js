...
function isCreditCardNumber(ccn) {
  const digitArr = ccn.toString().split('').reverse();

  const doubledArr = digitArr.map((digit, i) => (i % 2 === 0 ? +digit : digit * 2));
  const resultArr = doubledArr.map((digit) => (digit > 9 ? digit.toString().split('').reduce((a, b) => +a + +b, 0) : digit));

  const sum = resultArr.reduce((a, b) => a + b, 0);
...
function getCommonDirectoryPath(pathes) {
  const arrs = [];
  pathes.forEach((el) => arrs.push(el.split('/')));
  const commonArr = [];
  arrs.forEach((el) => commonArr.push(...el));
  const unique = Array.from(new Set(commonArr));
  const res = [];
  unique.forEach((el) => {
    if (arrs.every((elem) => elem.includes(el))) {
      res.push(el);
    }
  });
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < newMatrix.length; i += 1) {
    ...
    for (let j = 0; j < newMatrix[i].length; j += 1) {
      newMatrix[i][j] = 0;
      newMatrix[i][j] += m1[i][0] * m2[0][j] + m1[i][1] * m2[1][j] + m1[i][2] * m2[2][j];
    }
  }
  return newMatrix;
}
